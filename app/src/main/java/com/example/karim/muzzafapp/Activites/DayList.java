...
public class DayList extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mReference.child(shared.Email).addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
        ...
        mReferenceTimer.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if(dataSnapshot.hasChild(shared.Email)){
                    mReferenceTimer.child(shared.Email).addListenerForSingleValueEvent(new ValueEventListener() {
                        @Override
                        public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                            for(DataSnapshot shot :dataSnapshot.getChildren()){
                                TimerData td=shot.getValue(TimerData.class);
                                String ph=td.getPhoneNumber();
                                if(ph.equals(shared.Email))
                                    list.add(td);
                            }
